            ...
            <div className={classes.categoriesAndBlogs}>
                <div className={classes.categories}>
                    {categories.map((category) =>(
                        <span key={crypto.randomUUID} className={`${classes.category} ${activeCategory === category && classes.active}`}
                ...
                {filteredBlogs?.length> 0 ? <div className={classes.blogs}>
                    {filteredBlogs?.map((blog)=>(
                        <div key={blog._id} className={classes.blog}>
                            <Link to={`/blogDetails/${blog?._id}`}>
                                <img src={`http://localhost:5000/image/${blog?.photo}`} />
                            </Link>
                            <div className={classes.blogData}>
                                <div className={classes.categoryAndMetadata}>
                                    ...
                                    <div className={classes.metadata}>
                                        <MdOutlinePreview/>{blog.view} views
                                    </div>
                                    <div className={classes.metadata}>
